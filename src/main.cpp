...
uint256 hashGenesisBlock("0x0000059f9b4cff36275a2b411e20272e928ddf710c1bfab675731409bca768fa");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // urichie: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "urichie Signed Message:\n";
...
{
    // urichie: IsDust() detection disabled, allows any valid dust to be relayed.
...
bool CBlock::ConnectBlock(CValidationState &state, CBlockIndex* pindex, CCoinsViewCache &view, bool fJustCheck)
{
    ...
    // BIP16 didn't become active until August 1 2018
    int64 nBIP16SwitchTime = 1533099081;
...
{
    // urichie: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x0000059f9b4cff36275a2b411e20272e928ddf710c1bfab675731409bca768fa");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
		...
		const char* pszTimestamp = "urichie Genesis Block";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 501223881 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        ...
        block.nVersion = 1;
        block.nTime    = 1514768461;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 4175279;
        ...
        assert(block.hashMerkleRoot == uint256("0xfcdaf14276f308e1a07dfae1a35e454730834fe597e48374b650ddb88b83d40e"));
...
unsigned char pchMessageStart[4] = { 0xda, 0xdc, 0xb9, 0xfa }; // urichie: increase each by adding 6 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("urichieMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("urichieMiner : ProcessBlock, block not accepted");
    }
